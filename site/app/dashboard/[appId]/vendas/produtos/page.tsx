...
import { getStoreProducts, getVendasContext } from "@/lib/actions/vendas.actions";
...
export default async function ProdutosPage({ params }: { params: { appId: string } }) {
    ...
    try {
        products = await getStoreProducts(params.appId);
    } catch (error) {
        if (error instanceof ActionError) {
    ...
    return (
        <main className="mx-auto max-w-6xl px-5 py-8">
            ...
            {unavailableMessage ? (
                <Empty icon="!" title="Produtos indisponíveis" text={unavailableMessage} />
            ) : (
                <ProductsManager appId={params.appId} initial={products || []} />
            )}
        </main>
    );
}
