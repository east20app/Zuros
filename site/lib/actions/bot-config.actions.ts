...
async function ownedActiveApplication(appId: string) {
    ...
    const application = await databases.applications.findOne({ _id: appId, ownerId: discordId }, { botId: 1, status: 1, token: 1, serverId: 1 }).lean().catch(() => null);
    ...
    return { discordId, botId: application.botId, token: String(application.token || ""), serverId: String(application.serverId || "") };
}
