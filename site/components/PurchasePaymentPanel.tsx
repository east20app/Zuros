...
export function PurchasePaymentPanel({ cartId, initialStep }: { cartId: string; initialStep: string }) {
    ...
    return <div className="space-y-5 border-t border-zinc-800 pt-5">
        ...
        {!qr ? <div><p className="mb-4 text-sm text-[#949ba4]">Gere o PIX para concluir sua compra. O código expira em 30 minutos.</p><Button onClick={generate} disabled={pending} className="w-full">{pending ? <><Spinner /> Gerando PIX...</> : step === "waiting-payment" ? "Exibir pagamento PIX" : "Continuar para o pagamento"}</Button></div> : <div className="flex flex-col items-center gap-4"><div className="rounded-2xl bg-white p-3 shadow-[0_0_40px_-14px_rgba(88,101,242,.6)]">{/* eslint-disable-next-line @next/next/no-img-element */}<img src={qr} alt="QR Code PIX" className="h-52 w-52 rounded-lg" /></div>{price !== null && <p className="text-lg font-semibold text-white">Total: {price.toLocaleString("pt-BR", { style: "currency", currency: "BRL" })}</p>}<Button variant="secondary" className="w-full" onClick={() => copy(code).catch(() => push("Não foi possível copiar.", "error"))}>{copied ? "Código copiado!" : "Copiar código PIX"}</Button><span className="flex items-center gap-2 rounded-full border border-[#5865f2]/30 bg-[#5865f2]/10 px-3 py-1.5 text-xs text-[#7983f5]"><i className="h-2 w-2 animate-pulse rounded-full bg-[#5865f2]" />Aguardando confirmação do pagamento...</span></div>}
    </div>;
}
