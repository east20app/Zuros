...
export function LojaEditor({ value, onChange, persist, roles = [], channels = [] }: { value: Aggregate; onChange: (next: Aggregate) => void; persist: (next: Aggregate) => Promise<void>; roles?: DiscordGuildRole[]; channels?: DiscordGuildChannel[] }) {
    ...
    return (
        <div className="space-y-6">
            ...
            <Section title="Produtos" subtitle="A vitrine é formada por produtos; cada produto tem campos (opções de compra) com preço e estoque">
            ...
            <Modal open={Boolean(editingCampo)} onClose={() => setEditingCampo(null)} title={editingCampo ? `Campo: ${str(editingCampo.campo.name, "novo")}` : "Campo"}>
                {editingCampo && (
                    <CampoModalBody campo={editingCampo.campo} roles={roles} onChange={(campo) => setEditingCampo({ ...editingCampo, campo })} onCommit={commitCampo} onCancel={() => setEditingCampo(null)} />
                )}
            </Modal>
...
function RoleSelect({ roles, value, onChange, multiple = false }: { roles: DiscordGuildRole[]; value: string | string[]; onChange: (value: string | string[]) => void; multiple?: boolean }) {
    return <select multiple={multiple} className={`${inputClass} ${multiple ? "min-h-32" : ""}`} value={value} onChange={(event) => onChange(multiple ? Array.from(event.currentTarget.selectedOptions, (option) => option.value) : event.currentTarget.value)}>{!multiple && <option value="">Nenhum cargo</option>}{roles.map((role) => <option key={role.id} value={role.id}>{role.name}{role.managed ? " (integração)" : ""}</option>)}</select>;
}
        ...
        <div className="space-y-6">
            ...
            <div>
                <div className="mb-3 flex items-center justify-between">
                    ...
                    <Button size="sm" onClick={() => onOpenCampo(null, { id: generateId(), name: "", price: 0, emoji: null, pre_description: null, description: null, instructions: null, category_id: null, created_at: nowTs(), updated_at: nowTs(), advanced: {}, stock: [], cargos: { adicionar: [], remover: [] }, condicoes: { valorMin: null, valorMax: null, quantidadeMin: null, quantidadeMax: null } })}>Novo campo</Button>
                </div>
...
function CampoModalBody({ campo, roles, onChange, onCommit, onCancel }: {
    campo: Doc;
    ...
    return (
        <div className="space-y-5">
            <div className="grid gap-3 md:grid-cols-2">
                ...
                <Field label="Categoria (ID)"><input className={inputClass} value={str(campo.category_id)} onChange={(e) => set(["category_id"], e.target.value)} /></Field>
            ...
            <div className="rounded-xl border border-zinc-800 bg-black/20 p-4">
                ...
                <div className="grid gap-3 md:grid-cols-2">
                    <Field label="Cargos para adicionar" hint="Use Ctrl para selecionar vários"><RoleSelect multiple roles={roles} value={Array.isArray(cargos.adicionar) ? (cargos.adicionar as string[]) : []} onChange={(v) => set(["cargos", "adicionar"], v)} /></Field>
                    <Field label="Cargos para remover" hint="Use Ctrl para selecionar vários"><RoleSelect multiple roles={roles} value={Array.isArray(cargos.remover) ? (cargos.remover as string[]) : []} onChange={(v) => set(["cargos", "remover"], v)} /></Field>
                </div>
            </div>
            ...
            <Field label="Estoque (avançado)" hint="Lista de itens a entregar ou mapa centralizado. Não edite manualmente sem entender o formato.">
                <JsonEditor value={campo.stock ?? []} onChange={(v) => set(["stock"], v)} />
            </Field>
            <Field label="Opções avançadas do campo"><JsonEditor value={campo.advanced ?? {}} onChange={(v) => set(["advanced"], v)} /></Field>
