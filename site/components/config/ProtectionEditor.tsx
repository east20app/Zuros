                    ...
                    <div className="grid gap-2 rounded-lg border border-white/[.06] bg-black/20 p-3 sm:grid-cols-2">
                        ...
                        {Object.keys(advanced.obj).filter((key) => Array.isArray(advanced.obj[key])).map((key) => (
                            <div key={key} className="sm:col-span-2">
                                {/cargo|role/i.test(key) ? <Field label={key.replace(/_/g, " ").replace(/^./, (c) => c.toUpperCase())} hint="Use Ctrl para selecionar vários cargos"><select multiple className={`${inputClass} min-h-32`} value={asList(advanced.obj[key])} onChange={(event) => setInDoc([advanced.key, key], Array.from(event.currentTarget.selectedOptions, (option) => option.value))}>{roles.map((role) => <option key={role.id} value={role.id}>{role.name}</option>)}</select></Field> : /canal|channel|categoria/i.test(key) ? <Field label={key.replace(/_/g, " ").replace(/^./, (c) => c.toUpperCase())} hint="Use Ctrl para selecionar vários canais"><select multiple className={`${inputClass} min-h-32`} value={asList(advanced.obj[key])} onChange={(event) => setInDoc([advanced.key, key], Array.from(event.currentTarget.selectedOptions, (option) => option.value))}>{channels.map((channel) => <option key={channel.id} value={channel.id}>{channel.name}</option>)}</select></Field> : <ListField label={key.replace(/_/g, " ").replace(/^./, (c) => c.toUpperCase())} hint="Um item por linha" value={asList(advanced.obj[key])} onChange={(next) => setInDoc([advanced.key, key], next)} />}
                            </div>
                        ))}
                    </div>
                )}
