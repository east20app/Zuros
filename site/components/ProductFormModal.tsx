...
export function ProductFormModal({
    ...
    async function handleSubmit() {
        ...
        try {
            ...
            if (product) {
                await updateProduct(product.id, { name, runtimeEnvironment: runtime, runCommand, memoryMB: Number(memoryMB) || 256, prices, messageSettings });
                push("Produto atualizado.");
            } else {
                await createProduct(storeId, { name, runtimeEnvironment: runtime, runCommand, memoryMB: Number(memoryMB) || 256, prices, messageSettings });
                push("Produto criado.");
            }
            router.refresh();
            onClose();
                    ...
                    <div className="grid gap-3 lg:grid-cols-2">
                        ...
                        <div className="rounded-xl bg-[#313338] p-3 shadow-2xl ring-1 ring-black/50">
                            ...
                            <div className="border-l-4 border-emerald-500 bg-[#2b2d31] p-3 shadow-lg">
                                ...
                                {/* URL dinâmica do preview administrativo; não possui dimensões conhecidas para next/image. */}
                                {/* eslint-disable-next-line @next/next/no-img-element */}
                                {banner && <img src={banner} alt="Preview do banner" className="mt-3 max-h-40 w-full rounded object-cover" />}
