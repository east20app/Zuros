...
function getAppRoutes(appId: string): AppRoute[] {
    return [
        ...
        { label: "Vendas", href: `/dashboard/${appId}/vendas` },
    ];
}
                    ...
                    visibleApps.map((app) => {
                        ...
                        return (
                            <div key={app.id} className="overflow-hidden rounded-xl">
                                ...
                                {open && (
                                    <div id={`bot-nav-${app.id}`} className="ml-4 mt-1 space-y-0.5 border-l border-zinc-800/80 pl-3 animate-fade-in">
                                        {getAppRoutes(app.id).map(({ label, href }) => (
                                            <NavSubLink
                                                key={href}
                                                href={href}
                                                label={label}
