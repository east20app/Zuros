...
export function Icon({ name, className = "h-5 w-5", ...props }: { name: IconName } & Omit<SVGProps<SVGSVGElement>, "name">) {
    ...
    return <svg aria-hidden="true" viewBox="0 0 24 24" className={`${className} shrink-0 fill-none stroke-current stroke-[1.8]`} strokeLinecap="round" strokeLinejoin="round" {...props}>{paths[name]}</svg>;
}
