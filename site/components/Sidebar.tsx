...
type SidebarLink = { icon: SidebarIconName | "left"; label: string; href: string; badge?: "soon" | number; exact?: boolean };
...
export function Sidebar({
    ...
    const links: SidebarLink[] = admin
        ...
        : selectedBotId
        ? [
              { icon: "left", label: "Voltar ao Dashboard", href: "/dashboard", exact: true },
              { icon: "dashboard", label: "Visão geral do bot", href: `/dashboard/${selectedBotId}`, exact: true },
              { icon: "settings", label: "Configurar Loja", href: `/dashboard/${selectedBotId}/config/loja`, exact: true },
              { icon: "invoice", label: "Gerenciar Ticket", href: `/dashboard/${selectedBotId}/config/tickets`, exact: true },
              { icon: "dashboard", label: "Ver Rendimento", href: `/dashboard/${selectedBotId}/vendas`, exact: true },
              ...
              { icon: "invoice", label: "Carrinhos", href: `/dashboard/${selectedBotId}/vendas/carrinhos-abertos`, exact: true },
    ...
    const FullPanel = (
        <aside className="relative flex h-full min-h-0 flex-col overflow-hidden bg-background-dark px-3 py-4">
            ...
            {!admin && (
                <Link
                    href="/dashboard/account/extracts"
                    onClick={() => setOpen(false)}
                    className="mt-3 flex min-w-0 items-center justify-between gap-2 rounded-xl border border-emerald-500/15 bg-emerald-500/[.07] px-3 py-3 transition hover:border-emerald-500/30 hover:bg-emerald-500/[.1]"
                >
                    <span className="min-w-0 text-xs text-zinc-400">Saldo disponível</span>
                    <strong className="shrink-0 whitespace-nowrap text-sm text-emerald-300">{formatMoney(balance)}</strong>
                </Link>
            )}
