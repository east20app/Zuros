...
export function ReleaseUploader({ storeId, productId, disabled }: { storeId: string; productId: string; disabled: boolean }) {
    ...
    async function submit(event: React.FormEvent<HTMLFormElement>) {
        ...
        try {
            ...
            const result = await new Promise<{ version?: string; error?: string }>((resolve, reject) => {
                const xhr = new XMLHttpRequest();
                xhr.open("POST", `/api/products/${productId}/releases`);
                xhr.timeout = 120_000;
                xhr.upload.onprogress = (e) => e.lengthComputable && setProgress(Math.round((e.loaded / e.total) * 100));
                xhr.onerror = () => reject(new Error("Falha de rede durante o upload."));
                xhr.ontimeout = () => reject(new Error("O envio excedeu o tempo limite."));
                xhr.onload = () => {
                    let payload: { version?: string; error?: string } = {};
                    try { payload = JSON.parse(xhr.responseText) as typeof payload; } catch { /* handled below */ }
                    if (xhr.status < 200 || xhr.status >= 300) reject(new Error(payload.error || `Erro HTTP ${xhr.status}.`));
