...
function DynamicFields({ value, onChange, roles = [], channels = [], path = [] }: { value: Record<string, unknown>; onChange: (path: string[], value: unknown) => void; roles?: DiscordGuildRole[]; channels?: DiscordGuildChannel[]; path?: string[] }) {
    return <div className="grid gap-4 md:grid-cols-2">{Object.entries(value).map(([key, current]) => {
        ...
        if (typeof current === "string") { const color = /cor|color/i.test(key) && /^#[0-9a-f]{6}$/i.test(current); const secret = /token|secret|password|senha/i.test(key); return <Field key={id} label={labelFor(key)}><div className="flex gap-2">{color && <input aria-label={`Selecionar ${labelFor(key)}`} type="color" value={current} onChange={(e) => onChange(fieldPath, e.target.value)} className="h-10 w-12 cursor-pointer rounded border border-[#4e5058] bg-[#1e1f22] p-1" />}<input id={id} type={secret ? "password" : "text"} className={inputClass} value={current} onChange={(e) => onChange(fieldPath, e.target.value)} /></div></Field>; }
        ...
        if (Array.isArray(current) && /cargo|role/i.test(key) && roles.length) return <Field key={id} label={labelFor(key)} hint="Use Ctrl para selecionar mais de um cargo"><select multiple className={`${inputClass} min-h-32`} value={current.map(String)} onChange={(event) => onChange(fieldPath, Array.from(event.currentTarget.selectedOptions, (option) => option.value))}>{roles.map((role) => <option key={role.id} value={role.id}>{role.name}</option>)}</select></Field>;
        if (Array.isArray(current) && /canal|channel|categoria/i.test(key) && channels.length) return <Field key={id} label={labelFor(key)} hint="Use Ctrl para selecionar mais de um canal"><select multiple className={`${inputClass} min-h-32`} value={current.map(String)} onChange={(event) => onChange(fieldPath, Array.from(event.currentTarget.selectedOptions, (option) => option.value))}>{channels.map((channel) => <option key={channel.id} value={channel.id}>{channel.type === 4 ? "Categoria" : channel.type === 2 || channel.type === 13 ? "Voz" : "#"} {channel.name}</option>)}</select></Field>;
        if (Array.isArray(current) && current.every((item) => item === null || ["string", "number"].includes(typeof item))) return <Field key={id} label={labelFor(key)} hint="Um item por linha"><textarea id={id} rows={Math.max(3, Math.min(7, current.length + 1))} className={inputClass} value={current.join("\n")} onChange={(e) => onChange(fieldPath, e.target.value.split(/\r?\n/).map((item) => item.trim()).filter(Boolean))} /></Field>;
        if (Array.isArray(current)) return <fieldset key={id} className="rounded-lg border border-white/[.08] bg-[#232428] p-4 md:col-span-2"><legend className="px-2 text-sm font-semibold text-[#f2f3f5]">{labelFor(key)}</legend><div className="space-y-3">{current.map((item, index) => <div key={index} className="rounded-lg border border-white/[.06] bg-[#1e1f22] p-3">{item && typeof item === "object" && !Array.isArray(item) ? <DynamicFields value={item as Record<string, unknown>} path={[...fieldPath, String(index)]} onChange={onChange} /> : <span className="text-sm text-[#b5bac1]">{String(item)}</span>}<button type="button" onClick={() => onChange(fieldPath, current.filter((_, itemIndex) => itemIndex !== index))} className="mt-3 rounded-md bg-[#f23f43]/10 px-2.5 py-1.5 text-xs text-[#f87175] hover:bg-[#f23f43]/20">Remover</button></div>)}{current.length === 0 && <p className="text-sm text-[#949ba4]">Nenhum item configurado.</p>}</div></fieldset>;
...
function ConfiguracoesEditor({ value, roles, channels, onChange }: { value: Record<string, unknown>; roles: DiscordGuildRole[]; channels: DiscordGuildChannel[]; onChange: (path: string[], value: unknown) => void }) {
    const sections = Object.keys(SETTINGS_LABELS).filter((key) => key in value);
...
export function BotModuleEditor({ storeId, modulo }: { storeId: string; modulo: BotConfigModule }) {
    ...
    return <div><SaveBar dirty={dirty} saving={api.saving} onSave={() => void save()} /><div className="grid items-start gap-6 xl:grid-cols-[minmax(0,1fr)_380px]"><div className="min-w-0">{modulo === "loja" ? <LojaEditor value={draft} roles={roles} channels={channels} onChange={setDraft} persist={async (next) => { try { await api.save(next); push("Alteração salva com sucesso"); } catch (error) { push((error as BotConfigError).message || "Não foi possível salvar", "error"); await api.reload(); } }} /> : modulo === "configuracoes" ? <ConfiguracoesEditor value={draft} roles={roles} channels={channels} onChange={(path, value) => setDraft(updateAt(draft, path, value))} /> : <GenericModuleEditor modulo={modulo} value={draft} roles={roles} channels={channels} onChange={(path, value) => setDraft(updateAt(draft, path, value))} />}</div><DroxDiscordPreview value={draft} title={`Prévia · ${meta.name}`} /></div></div>;
}
