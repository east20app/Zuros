...
class SDKWrapper {
    ...
    public async updateCredentials(userId: string): Promise<UserSDKInstance | null> {
        const settingsDB = await databases.userSettings.findOne(
            ...
            { "settings.token_campos": 1, token_campos: 1 }
        ).lean();
        const token = String(settingsDB?.settings?.token_campos || settingsDB?.token_campos || "").trim();
        if (!token) {
            this.instances.delete(userId);
            return null;
        }

        const instance = new CamposCloudSDK({
            apiToken: token,
        });
        ...
        const sdkHttpClient = (instance as unknown as {
            axiosInstance?: { defaults: { timeout: number; maxBodyLength?: number; maxContentLength?: number } };
        }).axiosInstance;
        if (sdkHttpClient) {
            sdkHttpClient.defaults.timeout = REQUEST_TIMEOUT_MS;
            sdkHttpClient.defaults.maxBodyLength = Infinity;
            sdkHttpClient.defaults.maxContentLength = Infinity;
        }

        const userSDKInstance: UserSDKInstance = {
            instance,
