...
async function validateStoreCheckout(storeId: string) {
    ...
    if (!sdk?.isValid || !usage) throw new Error("Não foi possível verificar a disponibilidade para criar a aplicação. Entre em contato com o suporte.");
    if (usage.freeMemoryMB < 256) throw new Error("Não há recursos disponíveis para criar uma nova aplicação no momento. Entre em contato com o suporte.");
    return { store, owner };
}
