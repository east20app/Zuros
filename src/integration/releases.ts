...
export async function publishProductRelease(args: {
    ...
    try {
        ...
        await fs.writeFile(savedFilePath, strippedBuffer);
        saved = true;
        const result = await databases.products.updateOne(
            { _id: product._id },
            {
                $set: { lastReleaseCreatedVersion: nextRelease },
                $push: { releases: { version: nextRelease, date: new Date(), path: savedFilePath } },
            },
        );
        ...
        if (saved) {
            await fs.unlink(savedFilePath).catch(() => console.warn("Não foi possível remover o arquivo de release após erro."));
        }
        throw error;
    }
}
